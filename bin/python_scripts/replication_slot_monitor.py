...
def periodic_replication_slot_check(cursor,
        ...
        try:
            ...
            for record in cursor:
                if record[0] == debezium_slot_name:
                    ...
                    active_time_elapsed = round(
                        (datetime.datetime.now() -
                         active_checkpoint).total_seconds())
                    active = record[1]
                    if not active and active_time_elapsed > WARN_SLOT_INACTIVE_TIMEOUT:
                        logger.warning(
                            f"Replication slot is not active for {active_time_elapsed} seconds!"
                        )
                    ...
                    if bytes_diff < 1000:
                        ...
                        logger.info(
                            f"Debezium slot lag: {bytes_diff // Ki} KiB")
                    elif bytes_diff > Mi:
                        logger.info(
                            f"Debezium slot lag: {bytes_diff // Mi} MiB")
                    checked = True
            if not checked:
                logger.warning(
                    f"Debezium replication slot '{debezium_slot_name}' does not exist!"
                )
            else:
                checked = False
            ...
            logger.error(
                "Failed connecting to the PG server. Retrying after 5 seconds..."
            )
...
if __name__ == "__main__":
    ...
    try:
        periodic_replication_slot_check(cursor, args.debezium_slot_name,
                                        args.sleep)
